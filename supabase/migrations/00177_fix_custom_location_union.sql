-- 00176 special-cased "Custom location" to always override every other
-- selected area: picking Central (19) + Custom location (1) together
-- returned just the 1 custom-location activity instead of all 20. That
-- wasn't the intent — a custom-location activity should still be part of the
-- union when other areas are picked alongside it, exactly like every other
-- checkbox filter; it should only end up alone when "Custom location" is the
-- *only* thing picked.
--
-- The fix is to remove the case/when entirely. A custom-location activity's
-- own areas[] still carries its real (if unused) geocoded region — e.g.
-- 'central' for the one flagged so far — never the literal string 'custom',
-- since nothing ever writes that into a region column. So the plain
--   p_regions is null or x.areas && p_regions or x.is_custom_location
-- already gives both cases for free:
--   * p_regions = ['custom']            -> areas overlap never matches (no
--     activity's areas[] contains 'custom'), so only is_custom_location
--     activities pass — exactly "just the custom ones".
--   * p_regions = ['central','custom']  -> a plain Central activity matches
--     via the areas overlap, AND the custom one still matches (either via
--     areas overlap on its own real region, or via is_custom_location) — a
--     union of both, like any other two-way OR filter.
-- This is exactly 00175's original predicate, before 00176 touched it.
