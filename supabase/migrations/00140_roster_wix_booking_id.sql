-- 00140_roster_wix_booking_id.sql
--
-- provider_session_roster (00118) never exposed bookings.wix_booking_id, so
-- the vendor Bookings page's "Confirmed on Wix, but with no local booking
-- record" reconciliation panel (app/api/vendor/wix-session-bookings, wired
-- into frontends/vendor/src/pages/BookingsPage.tsx) had no way to tell which
-- of Wix's own attendees were ALREADY accounted for by a local booking and
-- which genuinely had none — so the frontend only ever showed the panel when
-- the local roster was completely empty. A session with even one local
-- booking silently suppressed the whole check, hiding any other attendees
-- Wix knows about that never got a local row (booked directly on Wix, or a
-- BabyBrain booking whose local insert failed after the Wix reservation went
-- through). Adding the column here lets the frontend correlate the two lists
-- and only show what's genuinely missing, regardless of how many local
-- bookings already exist.
--
-- Same body as 00118, only the new column.
...
returns table(
  ...
  waitlist_pay_invited boolean, group_primary_name text, wix_booking_id text
)
language plpgsql
