-- Free-tier parents can't see Packages/Make-up tokens on /profile (Plus-gated
-- PlusLock in frontends/parent/src/pages/dashboard.tsx) — the product copy on
-- that lock itself promises "On the free plan they come to you by email", but
-- the notifications this fires (make_up_token_issued, make_up_token_returned,
-- package_credit_returned) all linked to `/profile?tab=...`, which is exactly
-- the page a free-tier parent is locked out of. /book already lets any parent
-- redeem a package credit (auto-matched by provider, no id needed — see
-- PackageCard.bookHref) or a make-up token (via a `?token=` param — see
-- TokenRow's "Redeem" link) without ever touching /profile.
--
-- This deep-links each notification straight to /book instead, so the email
-- (see lib/emails/render.ts's new templates for these three types) is
-- actually usable by a free-tier parent. Falls back to the old /profile URL
-- only if the origin activity can't be resolved (shouldn't happen — same join
-- that already resolves v_activity_title).
