-- Packages could only ever be restricted to one activity (or none, meaning
-- "any"). A vendor with several related classes (e.g. three dance styles)
-- had no way to sell one pack redeemable across just those three without
-- also opening it to everything else. Widen to a list.

alter table public.packages add column if not exists activity_ids uuid[];

update public.packages
set activity_ids = array[activity_id]
where activity_id is not null and activity_ids is null;

alter table public.packages drop column if exists activity_id;

-- Same function as 00061, only the single-activity check swapped for a
-- membership check against the new array (null/empty still means "any").
