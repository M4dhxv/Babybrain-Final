...
// A stalled chunk request never rejects, and React Router 7 runs navigations in
// startTransition (old page stays up, no Suspense fallback) — so without a
// deadline a hung chunk looks like a dead link until a manual reload.
...
export function subscribeChunkLoads(listener: () => void): () => void {
...
export function chunkLoadsInFlight(): number {
  return inFlight;
}
