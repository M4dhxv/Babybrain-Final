...
import { geocode, slugify, VENDOR_CATEGORIES, type VendorCategory } from '@/lib/admin-create-provider';
...
export async function getProviderDetail(id: string): Promise<ProviderDetail | null> {
  ...
  const [locs, acts, cats] = await Promise.all([
    ...
    db.from('activities')
      .select('id, title, slug, category_id, age_min_months, age_max_months, price, is_published, description, external_booking_url')
  ...
  return {
    ...
    activities: (acts.data ?? []).map((a) => {
      const c = catById[a.category_id as unknown as number];
      return {
...
export async function updateProviderWithCatalogue(
  ...
  if (Object.keys(patch).length) {
    ...
    for (const k of ['business_name', 'description', 'contact_email', 'contact_phone',
                     'whatsapp', 'website', 'address', 'postal_code', 'status'] as const) {
