...
const FLOWS: FlowMeta[] = [
  ...
  { type: 'parent_welcome_paid', category: 'Parent', label: 'Welcome (Plus)', description: 'Sent when a parent who SIGNED UP on Plus completes payment (their plan choice is kept on the account through email confirmation). A parent who upgrades later gets no welcome.', wired: true, trigger: 'Stripe webhook (checkout.session.completed / customer.subscription.*) via welcomeIfSignedUpPaid() in lib/signup-plan.ts — only when the account carries intended_plan = plus.' },
  ...
  { type: 'make_up_token_issued', category: 'Parent', label: 'Make-up token issued', description: 'A make-up token was issued to the parent: automatically when a cancelled paid booking (or one against an expired pack) is compensated, or by the provider by hand from the Bookings page. Links straight to booking with it, so a free-tier parent can use it without the Plus-gated profile tab.', wired: true, trigger: 'DB triggers: compensate_cancelled_booking()\'s "Path 2" (migrations 00081, 00127) for auto-issued tokens, and notify_manual_make_up_token() (migration 00150) for tokens a provider issues by hand.' },
